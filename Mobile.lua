...
if game.PlaceId ~= 11468075017 then return end
...
LPH_NO_VIRTUALIZE(function()
    function library:Window(name)
        ...
        MainFrame.Position = UDim2.new(0.467886478, 0, 0.127, 0)
        ...
        MinimizeFrame.Parent = TopFrame
        MinimizeFrame.AnchorPoint = Vector2.new(0.5, 0.5)
        MinimizeFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        MinimizeFrame.BackgroundTransparency = 1.000
        MinimizeFrame.BorderSizePixel = 0
        MinimizeFrame.Position = UDim2.new(0.970000029, 0, 0.5, 0)
        MinimizeFrame.Size = UDim2.new(0, 25, 0, 25)
    
        MinimizeIcon.Name = "MinimizeIcon"
        MinimizeIcon.Parent = MinimizeFrame
...
-- // Setting Library \\ --
local window = library:Window("Dungeon [Beta]")
local Tabs = {
    Main = window:Tab("Project Slayers", "ClearMonster, KillAura", "rbxassetid://3926305904", Vector2.new(364, 484), Vector2.new(36, 36)),
}
-- // Services \\ --
...
local AutofarmDetails = {
    ...
    WeaponMethod = {'fist_combat','Sword_Combat_Slash','Scythe_Combat_Slash','claw_Combat_Slash'}
}
-- // Environment \\
...
local function GetClosest()
    ...
    for Index, Value in ipairs(Workspace.Mobs:GetChildren()) do
        if Value:FindFirstChildOfClass("Model") and Value:FindFirstChildOfClass("Model"):FindFirstChild("Humanoid").Health > 0 then
            local Distance = GetDistance(Value:FindFirstChildOfClass("Model"):GetModelCFrame())

            if Distance < Closest[1] then
                Closest = {Distance, Value:FindFirstChildOfClass("Model")}
            end
        end
    end
...
local function GetOrbs()
    ...
    if #Workspace.Mobs:GetChildren() == 0 then
        for Index, Orb in ipairs(Workspace.Map:GetChildren()) do
            if table.find(Orbs, Orb.Name) and Orb:FindFirstChildOfClass("MeshPart") then
                if Orb:FindFirstChildOfClass("MeshPart"):FindFirstChild("TouchInterest") then
                    Closest = Orb:FindFirstChildOfClass("MeshPart")
                end
            end
        end
    end
...
local function AutoCollectOrb()
    local Orb = GetOrbs()
    wait(1)
    if Orb then
        pcall(function()
            firetouchinterest(LocalPlayer.Character.HumanoidRootPart, Orb.TouchInterest.Parent, 0)
            wait()
            firetouchinterest(LocalPlayer.Character.HumanoidRootPart, Orb.TouchInterest.Parent, 1)
        end)
    end
end
...
local function IsDead()
    if not LocalPlayer:FindFirstChild("OuwigaharaSpectate") then
        return false
    end
    return true
end
...
Tabs.Main:AddToggle({
    Name = "Semi GodMode (Kamado Only)",
    Callback = function(value)
        Settings.SemiGodMode = value
    end
})
-- // Thread \\ --
RunService.Heartbeat:Connect(function()
    pcall(function()
        if (Settings.ClearMonster) then
        ...
        pcall(function()
            if (Settings.ClearMonster) then
                ...
                if (Closest) then
                    ...
                    until not Toggles.ClearMonster.Value or not Closest.Parent or not Closest.Humanoid.Health <= 0
                end
            end
        end)
    end
...
spawn(function()
    while wait() do
        if (Settings.AutoCollectChest) then
            pcall(AutoCollectDrop)
        end
    end
end)
spawn(function()
    while wait() do
        if (Settings.AutoCollectOrb) then
            pcall(AutoCollectOrb)
        end
    end
end)
spawn(function()
    while wait() do
        pcall(function()
            if (Settings.AutoRespawn.Value) then
                if IsDead() then
